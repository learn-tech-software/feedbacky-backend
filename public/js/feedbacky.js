...
function feedbackySendForm() {
    ...
    feedbackyData.formAreas.forEach(area => {
        var areaName = document.getElementsByClassName(area.name)[0].classList.value;
        var areaAnswer = document.getElementsByClassName(area.name)[0].value;
        if (area.minLength < areaAnswer.length && area.maxLength > areaAnswer.length) {
            formBodyFields.push({
            ...
            document.getElementsByClassName(area.name)[0].value = "Please, write something here..."
        }
    });
    if (formBodyFields.length > 0) {
